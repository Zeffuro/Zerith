            ...
            if (entry.name.endsWith('.json') && fullPath) {
                try {
                    ...
                    if (Array.isArray(data)) {
                        ...
                        if (isLikelyMacros) {
                            const picked = prompt(`Macro name to edit:\n${keys.join('\n')}`, keys[0] ?? '');
                            if (picked && Array.isArray(data[picked])) {
                                useProjectStore.getState().setActiveMacroName(picked);
                                useScriptStore.getState().setScript(validateScript(data[picked]));
                                useProjectStore.getState().setActiveFile(fullPath, validateScript(data[picked]));
                            }
                        }
                    }
                } catch (err) {
    ...
    return (
        <div>
            <div
                ...
                onMouseEnter={e => { if (!isSelected) e.currentTarget.style.backgroundColor = t.bg.hover; }}
...
export function Explorer() {
    ...
    return (
        <div style={{ padding: `${12 * uiScale}px 0`, height: '100%', backgroundColor: t.bg.panel, overflowY: 'auto' }} className="zerith-scrollbar">
            <div style={{ padding: `0 ${12 * uiScale}px`, display: 'flex', alignItems: 'center', gap: '8px', color: t.text.primary, marginBottom: `${16 * uiScale}px`, fontSize: '0.9em', fontWeight: 'bold', letterSpacing: '1px' }}>
                EXPLORER
            </div>

            {projectPath ? (
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: `0 ${12 * uiScale}px`, marginBottom: '8px', color: t.text.muted, fontSize: '0.85em', textTransform: 'uppercase', fontWeight: 'bold' }}>
                ...
                <div style={{ padding: `0 ${12 * uiScale}px`, fontSize: 'inherit', color: t.text.faint, fontStyle: 'italic' }}>
                    No project opened.
                </div>
            )}
        </div>
